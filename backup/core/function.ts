...
import { z } from 'zod';
import type { Function, FunctionOptions, FunctionResult } from '../types/function';
 ...
 * @returns JSON 스키마
 */
function zodToJsonSchema(schema: z.ZodObject<any>): FunctionSchema['parameters'] {
...
/**
 * 일반 함수를 Function 객체로 변환
 * @param name 함수 이름
 * @param fn 변환할 함수
 * @param description 함수 설명 (선택 사항)
 * @returns Function 객체
 */
export function functionFromCallback(
  ...
  const parameters: FunctionSchema['parameters'] = {
    type: 'object',
    properties: {},
    required: []
  };
